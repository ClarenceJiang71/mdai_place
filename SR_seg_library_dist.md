...
# Code To Convert MD.ai annotations to SR/SEG
...
class SR_SEG_Export:
  ...
  Inputs:
    ...
    `combine_label_groups` - If `True` then each SEG file includes the annotations from every label group for that series
                                and will be visually separated by color -- WARNING THIS WILL 8x THE FILE SIZE --
                             If `False` then a different SEG file will be created for each different label group annotation
                            (only applies to SEG output)
  ...
  '''
  def __init__(self, output_format, annotation_json, metadata_json, combine_label_groups=False, output_dir=None):
  ...
  def img_insert(self, row, ds):
    ...
    if not np.isscalar(data):
      ...
      if row['labelId'] not in self.seen_labels:
        ...
        if self.combine:
          segment1.RecommendedDisplayCIELabValue = self.colors[self.label_groups.index(row['labelGroupName']) % len(self.colors)]
      ...
      for study_uid in self.studies:
        ...
        for series_dict in dicom_hier:
          for series_uid in series_dict:
            ...
            # ds.StudyDate = # Should be generated from placing tags
            ds.SeriesDate = str(date)  # Series Date
            ds.ContentDate = str(date)  # Content Date
            # ds.StudyTime = # Should be generated from placing tags
            ...
            for annotator_id in annotators:
              ...
              # Colors from 12 section color wheel
              self.colors = [[32,  79,  128], [32,  180,  56]]
              for label_group_set in label_group_sets:
                ...
                if self.included_sops:
                  if self.combine:
                    ...
                    ds.save_as(f'{os.getcwd()}/{out_dir}/DICOM_SEG_{dataset_id}_label_group_{label_group_set[0]}_series_{series_uid}_annotator_{annotator_id}.dcm', False)

```

# Change The output format ("SR"/"SEG") and input your json files
The output will be in a folder called "SR(/SEG)_OUTPUT"


```python
SR_SEG_Export(output_format= #SR OR SEG,
              annotation_json= #YOUR FILE HERE,
              metadata_json= #YOUR FILE HERE)
```
